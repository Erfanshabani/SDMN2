...
sudo ip netns exec Router1 ip link set veth4 up
...
sudo ip link set veth6-br up
sudo ip netns exec NS4 ip link set veth6 up

#Adding Routes:
sudo ip netns exec NS1 ip route add 0.0.0.0/0 via 172.0.0.1
sudo ip netns exec NS2 ip route add 0.0.0.0/0 via 172.0.0.1
sudo ip netns exec NS3 ip route add 0.0.0.0/0 via 10.10.0.1
sudo ip netns exec NS4 ip route add 0.0.0.0/0 via 10.10.0.1
